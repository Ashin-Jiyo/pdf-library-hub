...
const GuestUpload: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.title.trim().length > 18) {
      toast.error('Title must be 18 characters or less');
      return;
    }
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          <label style={{
            ...
            Title * <span style={{ color: '#7f8c8d', fontWeight: 400 }}>({formData.title.length}/18)</span>
          </label>
          <input
            type="text"
            ...
            style={{
              width: '100%',
              padding: '1rem',
              border: `2px solid ${formData.title.length > 18 ? '#e74c3c' : '#e0e6ed'}`,
              borderRadius: '12px',
              fontSize: '1rem',
              transition: 'all 0.3s ease',
              boxSizing: 'border-box',  
            ...
            onBlur={(e) => {
              e.target.style.borderColor = formData.title.length > 18 ? '#e74c3c' : '#e0e6ed';
              e.target.style.boxShadow = 'none';
            }}
          />
          {formData.title.length > 15 && (
            <p style={{
              margin: '0.5rem 0 0 0',
              fontSize: '0.8rem',
              color: formData.title.length > 18 ? '#e74c3c' : '#f39c12'
            }}>
              {formData.title.length > 18 ? 'Title is too long!' : 'Title is getting long'}
            </p>
          )}
        </div>
        ...
        <div>
          ...
          <p style={{
            ...
            <a
              ...
              onClick={(e) => {
                e.preventDefault();
                // Navigate to the category request page
